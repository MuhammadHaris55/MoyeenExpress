...
// import 'dart:convert';
//
// List<List<Product>> productFromJson(String str) => List<List<Product>>.from(json.decode(str).map((x) => List<Product>.from(x.map((x) => Product.fromJson(x)))));
//
// String productToJson(List<List<Product>> data) => json.encode(List<dynamic>.from(data.map((x) => List<dynamic>.from(x.map((x) => x.toJson())))));
//
// class Product {
//   Product({
//     required this.id,
...
//     discountedPercentage: json["discounted_percentage"] == null ? null : json["discounted_percentage"],
//     status: json["status"],
